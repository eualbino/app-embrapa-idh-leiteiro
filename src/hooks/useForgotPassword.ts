...
export function useForgotPassword() {
  ...
  const validateOtp = async (otp_code: string) => {
    try {
      ...
      if (status === 400) {
        errorMessage = responseData?.err || responseData?.message || 'Código inválido.';
      } else if (status === 404) {
        errorMessage = responseData?.message || 'Email ou código não encontrado. Solicite um novo código.';
      }
